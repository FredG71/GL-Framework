...
void main()
{
	vec3 LightPos = vec3(10.f, 5.f, 2.f);
	vec3 LightDir = normalize(LightPos - vPos);
	vec4 Diffuse = Kd_Diffuse * max(dot(LightDir, normalize(vNormal)), 0);
	float Alpha = max(dot(LightDir, normalize(vNormal)), 0);
